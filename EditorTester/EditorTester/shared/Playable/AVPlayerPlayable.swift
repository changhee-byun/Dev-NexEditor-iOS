...
public class AVPlayerPlayable: NSObject, Playable {
    ...
    public var duration: CMTime { return player.currentItem?.duration ?? CMTime.zero }
    public var currentTime: CMTime { return player.currentItem?.currentTime() ?? CMTime.zero }
    ...
    override public func observeValue(forKeyPath keyPath: String?,
        ...
        if keyPath == #keyPath(AVPlayerPlayable.player.currentItem.duration) {
            if let duration = change?[.newKey] as? CMTime, duration != CMTime.indefinite {
        ...
        if #available(iOS 10.0, *) {
            if keyPath == #keyPath(AVPlayerPlayable.player.timeControlStatus) {
                if let status = change?[.newKey] as? NSNumber {
                    let playing = status.intValue == AVPlayer.TimeControlStatus.playing.rawValue
