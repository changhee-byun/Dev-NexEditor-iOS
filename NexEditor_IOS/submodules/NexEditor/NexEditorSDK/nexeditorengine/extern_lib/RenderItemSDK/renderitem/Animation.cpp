...
void Animation::load(RenderItem* ri, rapidxml::xml_node<> *node){
	...
	while (frame_node){
		...
		RenderItemContext* ctx = ri->getContext();
		...
		while (image_node){
			...
			if (NULL == image){
				...
				if(0 == (*ctx->file_ops_.ploadfunc)(&poutput, &length, (char*)(ri->getNamespace() + std::string("/") + str_for_file).c_str(), ctx->file_ops_.cbdata_for_loadfunc)){
