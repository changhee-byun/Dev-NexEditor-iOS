...
void nexFindHomographyID(Vector2 p0, Vector2 p1, Vector2 p2, Vector2 p3, Matrix3& ret){
    ...
    float det = 0.1e1 / (-t1 + t2 - t0 + t3 - t4 + t5);
    
    ret[0] = (p0.x * (t0 - t3 - t5 + t4) + (t6 - t7 - t0) * p3.x + p1.x * t1) * det;
    ret[1] = (p0.x * (t0 - t3 - t2 + t1) + t7 * (p3.x - p2.x) + p1.x * (t3 - t1)) * det;
    ret[2] = p0.x;
    ret[3] = ((-t8 + t9 + t5 - t0) * p3.y + p0.y * (t0 - t2 + t1 - t5)) * det;
    ret[4] = (t9 * (-p3.y + p2.y) + p0.y * (t1 - t3 + t4 - t5) + t0 * p3.y - p1.y * t1) * det;
    ret[5] = p0.y;
    ret[6] = (-t8 + t9 + t6 - t7 + t1 - t2 - t3 + t4) * det;
    ret[7] = (-p0.x * p3.y + t8 + p0.y * p3.x - t6 + t4 - t5 - t2 + t0) * det;
    ret[8] = 1;
}

void nexFindHomography(Vector2* src, Vector2* dst, Matrix3& result){

    Matrix3 mat0, mat1;

    nexFindHomographyID(src[0], src[1], src[2], src[3], mat0);
    nexFindHomographyID(dst[0], dst[1], dst[2], dst[3], mat1);

    result = mat1 * mat0.invert();
}
...
// M^-1 = adj(M) / det(M)
///////////////////////////////////////////////////////////////////////////////
Vector4* D3DXVec4Cross(Vector4 *pout, Vector4 *pv1, Vector4 *pv2, Vector4 *pv3)
{
...
Matrix4& Matrix4::perspective(float fov_degrees, float aspect, float z_near, float z_far) {
	...
	set(
		...
		0.0, 0.0, 2.0f * z_far*z_near / (z_near - z_far), 0.0
	);

	return *this;
}

Matrix4& Matrix4::lookAt(const Vector3& eye, const Vector3& look, const Vector3& up){
