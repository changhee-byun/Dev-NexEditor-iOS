...
GLuint Img::loadImageRaw(const char* pixels, int width, int height){

	GLuint image = SOIL_create_OGL_texture((const unsigned char* const)pixels, width, height, 4, 0, SOIL_FLAG_MULTIPLY_ALPHA);
...
GLuint Img::loadImage(const char* filename, char* src, int length, float* width, float* height){
	...
	int w, h;
	GLuint image = SOIL_load_OGL_texture_from_memory((const unsigned char *)src, length, 0, 0, SOIL_FLAG_MULTIPLY_ALPHA | SOIL_FLAG_DDS_LOAD_DIRECT, &w, &h, NULL);
...
void Img::loadShaderForUI(){
	...
	glLinkProgram(program); CHECK_GL_ERROR(); CHECK_GL_ERROR();
	glDetachShader(program, vertex_shader); CHECK_GL_ERROR();
	glDetachShader(program, fragment_shader); CHECK_GL_ERROR();
