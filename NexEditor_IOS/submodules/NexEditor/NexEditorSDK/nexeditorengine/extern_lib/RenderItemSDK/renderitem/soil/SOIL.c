	...
	)
{
	...
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	{
		/*	1st try direct loading of the image as a DDS file
			...
			not be flipped, etc.	*/
		tex_id = SOIL_direct_load_DDS( filename, reuse_texture_ID, flags, 0, pwidth, pheight );
		if( tex_id )
		{
			/*	hey, it worked!!	*/
			return tex_id;
	...
	img = SOIL_load_image( filename, pwidth, pheight, &channels, force_channels );
	...
	)
{
	...
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	{
		...
		tex_id = SOIL_direct_load_DDS_from_memory(
				...
				reuse_texture_ID, flags, 0, pwidth, pheight );
		if( tex_id )
		{
			/*	hey, it worked!!	*/
			return tex_id;
...
unsigned int SOIL_direct_load_DDS(
		...
		unsigned int reuse_texture_ID,
		int flags,
		int loading_as_cubemap, int* pwidth, int* pheight )
{
	FILE *f;
	...
	tex_ID = SOIL_direct_load_DDS_from_memory(
		...
		reuse_texture_ID, flags, loading_as_cubemap, pwidth, pheight );
