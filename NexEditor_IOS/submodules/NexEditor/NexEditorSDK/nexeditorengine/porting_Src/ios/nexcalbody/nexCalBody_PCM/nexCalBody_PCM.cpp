...
typedef enum
{
	...
	NEXCALBODY_AUDIO_MULTICHANNEL_5POINT1_MORE_TO_5POINT1 = 0x2,		// 5.1 channel 이하는 Stereo 로 설정, 5.1 channel 이상은 5.1 channel 로 설정 //G2 요구 사항.
	NEXCALBODY_AUDIO_MULTICHANNEL_ANY_TO_ANY = 0x20,				// Multi Channel Full 지원 
...
NXINT32 nexCALBody_Audio_PCM_GetProperty( NXUINT32 uProperty, NXINT64 *puValue, NXVOID* pUserData )
{
	switch (uProperty)
	{
...
NXINT32 nexCALBody_Audio_PCM_SetProperty( NXUINT32 uProperty, NXINT64 uValue, NXVOID* pUserData )
{
	return 0;
}
                                   ...
                                   NXVOID** ppUserData )
{
	...
	pAudioUsrData = (PNEXADECUsrData)malloc( sizeof(NEXADECUsrData) );
	if (!pAudioUsrData)
	{
//		nexCAL_DebugPrintf("Error Memory Allocation\n");
	...
	pPCMInfo = (PNEXPCMInfo)malloc( sizeof(NEXPCMInfo) );
	if (!pPCMInfo)
	{
//		nexCAL_DebugPrintf("Error Memory Allocation\n");
		*ppUserData = NULL;
	...
	pAudioUsrData->m_puExtraData = pPCMInfo;
...
*/
	...
	if( (pAudioUsrData->m_uBitsPerSample == 32) || (pAudioUsrData->m_uBitsPerSample == 64) )
	{
		...
		if( (pAudioUsrData->m_uAudioOTI == eNEX_CODEC_A_PCM_FL32LE) || (pAudioUsrData->m_uAudioOTI == eNEX_CODEC_A_PCM_FL32BE) )
		{
			...
			for(i=0; i<iLen/4; i++)
			{
				iPCM64 = (NXINT64)(pPCMFL[i] * 2147483648);
		...
		else if( (pAudioUsrData->m_uAudioOTI == eNEX_CODEC_A_PCM_FL64LE) || (pAudioUsrData->m_uAudioOTI == eNEX_CODEC_A_PCM_FL64BE))
		{
			...
			for(i=0; i<iLen/8; i++)
			{
				iPCM64 = (NXINT64)(pPCMFL64[i] * 2147483648);
				if(iPCM64 > (NXINT64)0x000000007fffffff)
					pPCM32[j++] = 2147483647;
				else if(iPCM64 < (NXINT64)0xffffffff80000000)
					pPCM32[j++] = -2147483648;
...
//				nexCAL_DebugPrintf("[nexCalBody_PCM %d] Out of MaxPCMSize !!! %d, %d \n", __LINE__, uMaxPCMSize, *piWrittenPCMSize);
			...
			case 3 :
				{
					switch (nOutBps)
					{
						case 32:
							{
								...
								for (i=0; i<uActualSize; i += pAudioUsrData->m_uNumOfChannels)
								{
									...
									if (iPCM64 > (NXINT64)0x000000007fffffff)
										*pMixedPCM++ = 2147483647;
									else if (iPCM64 < (NXINT64)0xffffffff80000000)
										*pMixedPCM++ = -2147483648;
									else
						...
						case 24:
							{
								int j = 0;
								//NXUINT8 tmpPCM8[8192 *4/*int*/ * 3/*ch*/]; //24bps다운 믹스 32 bit 연산위한 버퍼,  추후 stack -> heap memory 로....pjb test
								NXUINT8 *tmpPCM8 = (NXUINT8 *)malloc(8192 * 4 * 3);
			...
			case 4 :
				{
					switch (nOutBps)
					{
						...
						case 24:
							{
								int j = 0;
								//NXUINT8		tmpPCM8[8192 *4/*sizeof(int)*/ * 4/*ch*/]; //24bps다운 믹스 32 bit 연산위한 버퍼,  추후 stack -> heap memory 로....pjb test
								NXUINT8     *tmpPCM8 = (NXUINT8 *)malloc(8192 * 4 * 4);
								NXUINT8     *pPCM8 = (NXUINT8*) pDest;
								NXUINT8     *pMixedPCM = (NXUINT8*) pDest;
								NXINT64     iPCM64 = 0;
								NXINT32     *pTmpPCM32;
			...
			case 5 :		// 5 ch : L,R,C,Ls,Rs
				{
					switch (nOutBps)
					{
						...
						case 24:
							{
								int j = 0;
								//NXUINT8		tmpPCM8[8192 *4/*sizeof(int)*/ * 6/*ch*/]; //24bps다운 믹스 32 bit 연산위한 버퍼,  추후 stack -> heap memory 로....pjb test
								NXUINT8     *tmpPCM8 = (NXUINT8 *)malloc(8192 * 4 * 6);
								NXUINT8     *pPCM8 = (NXUINT8*) pDest;
								NXUINT8     *pMixedPCM = (NXUINT8*) pDest;
								NXINT64     iPCM64 = 0;
								NXINT32     *pTmpPCM32;
				...
				break;
			case 6 :	/* ignore LFE */ // 5.1 ch 로 : L,R,C,LFE,Ls,Rs
				{
					switch( nOutBps )
					{
					case 32:
			...
			case 8 :	/* ignore rLs, rRs, LFE */		/* 3/4+lfe : L,R,C,Ls,rLs,rRs,Rs,LFE	*/
				{
					switch (nOutBps)
					{
						...
						case 24:
							{
								int j = 0;
								//NXUINT8		tmpPCM8[8192 *4/*sizeof(int)*/ * 8/*ch*/]; //24bps다운 믹스 32 bit 연산위한 버퍼,  추후 stack -> heap memory 로....pjb test
								NXUINT8     *tmpPCM8 = (NXUINT8 *)malloc(8192 * 4 * 8);
								NXUINT8     *pPCM8 = (NXUINT8*) pDest;
								NXUINT8     *pMixedPCM = (NXUINT8*) pDest;
								NXINT64     iPCM64 = 0;
								NXINT32     *pTmpPCM32;
				...
				case 32:
					{
						NXINT32     *pPCM32 = (NXINT32*) pDest;
						NXINT32     *pMixedPCM = (NXINT32*) pDest;
						NXINT32     *iPCM32Temp = (NXINT32 *)malloc(8192 * 4 * 6);
						int count = 0;
	...
	case 24:
				{
					NXUINT8		*tmpPCM8 = (NXUINT8 *)malloc(8192 * 4 * 6); //MaxBufferSize * sizeof(int) * channel 
					NXUINT8		*pPCM8 = (NXUINT8*) pDest;
					NXUINT8		*pMixedPCM = (NXUINT8*) pDest;
					NXINT32		*pTmpPCM32; 
					int count = 0;
				...
				case 24:
					{
						...
						NXUINT8     *iPCM8Temp = (NXUINT8 *)malloc(8192 * 4 * 6);
						int count = 0;
				...
				case 16:
					{
						...
						NXINT16     *iPCM16Temp = (NXINT16 *)malloc(8192 * 4 * 6 );
						int count = 0;
		...
		{
			//*piWrittenPCMSize *= 3;	//실제 output pcm size 가 6ch 이여야 합, 상위 코드에서 이미 2ch output 을 위해  pcm size *2 를 한 상태이므로 *3 = 6 으로 설정
			*piWrittenPCMSize *= 6;	//실제 output pcm size 가 6ch 이여야 합

// 7 /* ignore rLs, rRs */			/* 3/4     : L,R,C,Ls,Rs,rLs,rRs		*/
// 8 /* ignore rLs, rRs, LFE */		/* 3/4+lfe : L,R,C,LFE,Ls,Rs,rLs,rRs,	*/ // 5.1 ch 로 : L,R,C,LFE,Ls,Rs
			switch (nOutBps)
			{
				case 32:
					{
				...
				case 24:
					{
						...
						//for(i=0; i< uActualSize * 3; i++ ) // i 는 입력의 Samples
						//for(j=0; j< uActualSize * 3; ) // j 출력의 Byte 수
						for (i=0,j=0; i< uActualSize; i++)
						{
							tmpPCM8[j++] = 0;
							tmpPCM8[j++] = *pPCM8++;
...
NXINT32 nexCALBody_Audio_PCM_GetInfo	( unsigned int uIndex
