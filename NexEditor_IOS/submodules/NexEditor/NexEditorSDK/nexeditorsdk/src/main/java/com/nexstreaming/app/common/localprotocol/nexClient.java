...
public class nexClient {
    ...
    public void sendReqMessage(short command ,int sessionID ,byte[] message) throws IOException{
        LocalSocket socket = mSocket;
        if (socket == null) {
            Log.w(TAG,"'send' request on closed Socket ");
            throw new ClosedChannelException();
        ...
        socket.getOutputStream().write(nexProtocol.makeRequestHeader(command, sessionID, message.length));
        socket.getOutputStream().write(message);
    }

    public void sendResMessage(short command ,int errorCode ,byte[] message) throws IOException{
        LocalSocket socket = mSocket;
        if (socket == null) {
            Log.w(TAG,"'send' request on closed Socket ");
            throw new ClosedChannelException();
        ...
        socket.getOutputStream().write(nexProtocol.makeResponseHeader(command, errorCode, message.length));
        socket.getOutputStream().write(message);
    }
    ...
    public nexProtocol.HDRnData receive(short command, int sessionID, boolean req ) throws IOException{
        ...
        final ByteBuffer recv = ByteBuffer.allocate(nexProtocol.PACKET_HEADER_SIZE);
        nexProtocol.receive(socket, recv.array(), recv.array().length);
        nexProtocol.Header hdr= nexProtocol.checkHeader(recv,sessionID,req);
        if( !hdr.isOk ){
            Log.w(TAG,"'receive' Header is invalid. error="+hdr.errorCode);
            throw new ClosedChannelException();
        }
        ...
        nexProtocol.HDRnData ret = new nexProtocol.HDRnData(hdr);
        ret.data = new byte[hdr.messageSize];
        nexProtocol.receive(socket, ret.data, ret.data.length);
        return ret;
    }
