...
public class nexProtocol {
    private static final int PACKET_SIGNATURE = 0x4e657850; //'NexP'
    static final int PACKET_HEADER_SIZE = 16; //magic, cmd , randsum, size
    ...
    public static byte[] makeSenderHeader(int command  ,int messageSize){
        ...
        handshake.putInt(random.nextInt());
    ...
    public static Header checkHeader(ByteBuffer buff,int sessionId, boolean req ){
        Header hdr = new Header();
        ...
        if( req ) {
            hdr.randsum = buff.getInt();
            if( req && hdr.randsum != sessionId){
                hdr.errorCode = nexProtoErrorCode.InvalidSSID.getValue();
                return hdr;
            }
        }else{
