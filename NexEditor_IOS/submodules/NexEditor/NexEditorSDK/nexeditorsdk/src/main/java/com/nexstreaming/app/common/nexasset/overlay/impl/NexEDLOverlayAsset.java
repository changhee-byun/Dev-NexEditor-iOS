...
public class NexEDLOverlayAsset extends AbstractOverlayAsset {
    ...
    public void setMode(int change_mode){
    ...
    public AwakeAsset onAwake(LayerRenderer renderer, final RectF bounds, final String effectOptions, Map<String,String> unencodedEffectOptions ){
        ...
        return new AwakeAsset() {
            ...
            public void onAsleep(LayerRenderer renderer) {
                ...
                //delete NexEDL proxy..parameter will be engine id
                NexEditor editor = EditorGlobal.getEditor();
                if( editor!=null ) {

                    if(effect_id_[renderer.getRenderMode().id] >= 0) {

                        editor.releaseNexEDLJ(effect_id_[renderer.getRenderMode().id], renderer.getRenderMode().id);
            ...
            public void onRender(LayerRenderer renderer, OverlayMotion motion, int startTime, int endTime ) {
                ...
                if(effect_id_[renderer.getRenderMode().id] < 0) {
                    ...
                    NexEditor editor = EditorGlobal.getEditor();
                    if( editor!=null ) {
                        effect_id_[renderer.getRenderMode().id] = editor.createNexEDLJ(getItemInfo().getId(), renderer.getRenderMode().id);
                    }
                }
                if(effect_id_[renderer.getRenderMode().id] >= 0)
                    renderer.drawNexEDL(effect_id_[renderer.getRenderMode().id], currentEffectOptions, renderer.getCurrentTime(), startTime, endTime, currentBound.left, currentBound.top, currentBound.right, currentBound.bottom, renderer.getAlpha(), mode);
            }
