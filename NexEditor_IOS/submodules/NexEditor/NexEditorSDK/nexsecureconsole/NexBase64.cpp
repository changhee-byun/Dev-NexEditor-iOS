...
int BitsToBase64(unsigned char* pOut, int nOutLen, unsigned char* pIn, int nInLen)
{
	...
	for (; nInLen >= 3; nInLen -= 3)
	{
		...
		*pOut++ = base64digits[pIn[0] >> 2];
		*pOut++ = base64digits[((pIn[0] << 4) & 0x30) | (pIn[1] >> 4)];
		*pOut++ = base64digits[((pIn[1] << 2) & 0x3c) | (pIn[2] >> 6)];
		*pOut++ = base64digits[pIn[2] & 0x3f];
		pIn += 3;
		nConvertLen += 4;
    }
    if (nInLen > 0)
    {
		...
		*pOut++ = base64digits[pIn[0] >> 2];
		...
		if (nInLen > 1)
			fragment |= pIn[1] >> 4;
		*pOut++ = base64digits[fragment];
		*pOut++ = (nInLen < 2) ? '=' : base64digits[(pIn[1] << 2) & 0x3c];
		*pOut++ = '=';
		nConvertLen +=3;
	}
	*pOut = '\0';
...
int Base64ToBits(unsigned char* pOut, int nOutLen, unsigned char* pIn, int nInLen)
{
	...
	// kyle.jung_130719_initialize memory
	memset(pOut, 0x00, sizeof(unsigned char)*nOutLen);
