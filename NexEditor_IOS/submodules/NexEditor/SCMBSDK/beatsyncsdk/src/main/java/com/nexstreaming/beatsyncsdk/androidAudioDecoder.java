...
/**
 * Created by jeongwook.yoon on 2018-09-17.
 */
...
public class androidAudioDecoder {
    ...
    public interface OnReceivedDataCallback{
        ...
        void onEos(androidAudioDecoder ad);
        void onCancel(androidAudioDecoder ad);
    }
    ...
    private  final class AudioDecProcess extends AsyncTask<androidAudioDecoder,Void,Integer> {
        private  int num;
        private androidAudioDecoder ad;
        private OnReceivedDataCallback callback;
        AudioDecProcess(int num, androidAudioDecoder ad, OnReceivedDataCallback callback){
            this.num = num;
            this.ad = ad;
        ...
        @Override
        protected Integer doInBackground(androidAudioDecoder... params) {
            int rval = Decoder();
            return rval;
        }
    ...
    public void decode(){
        ...
        mDecoder.start();
        new AsyncTask<androidAudioDecoder,Void,Integer>(){
            androidAudioDecoder target;
            @Override
            protected Integer doInBackground(androidAudioDecoder... params) {
                target = params[0];
                int rval = Decoder();
